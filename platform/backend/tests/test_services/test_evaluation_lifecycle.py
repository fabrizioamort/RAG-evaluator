...
# Suppress pytest collection warnings for models
TestCase.__test__ = False  # type: ignore
TestSet.__test__ = False  # type: ignore


async def create_setup_data(db_session: AsyncSession, case_count: int = 3) -> tuple[Evaluation, list[uuid.UUID]]:
...
def setup_common_mocks(stack: ExitStack, patches: list, mock_rag, artifact_ids):
    ...
    mock_checkpoint.create_job = AsyncMock(side_effect=lambda eid, total: EvaluationJob(evaluation_id=eid, progress_total=total, progress_current=0))
...
@pytest.mark.asyncio
async def test_evaluation_full_lifecycle(db_session: AsyncSession, caplog) -> None:
    ...
    mock_rag = MagicMock()
    mock_rag.query_with_trace = AsyncMock(return_value={
        "answer": "Mock Answer",
    ...
    with ExitStack() as stack:
        ...
        res = await db_session.execute(select(EvaluationResult).where(EvaluationResult.evaluation_id == evaluation.id))
        results = res.scalars().all()
        assert len(results) == 3


@pytest.mark.asyncio
async def test_evaluation_cancel_lifecycle(db_session: AsyncSession, caplog) -> None:
...
@pytest.mark.asyncio
async def test_evaluation_pause_resume_lifecycle(db_session: AsyncSession, caplog) -> None:
    ...
    with ExitStack() as stack:
        ...
        async def query_side_effect_resume(*args, **kwargs):
            return original_return
