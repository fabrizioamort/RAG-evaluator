...
async def create_legacy_index(session, eval: Evaluation, kb: KnowledgeBase, rag_config: RAGConfig) -> KnowledgeBaseIndex:
    ...
    index = KnowledgeBaseIndex(
        ...
        created_at=eval.started_at or datetime.now(timezone.utc),
        build_completed_at=eval.started_at or datetime.now(timezone.utc),
    )
...
async def migrate():
    ...
    async with get_db_context() as session:
        ...
        for eval in evaluations:
            try:
                ...
                if existing_index:
                    ...
                    new_index = await create_legacy_index(session, eval, eval.knowledge_base, eval.rag_config)
        ...
        print(f"Migration complete. Migrated {migrated_count} evaluations. Created {created_indices} new legacy indexes.")
