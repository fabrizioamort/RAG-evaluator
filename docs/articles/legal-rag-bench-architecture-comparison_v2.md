...
*I built an evaluation platform for RAG systems. Then I pointed it at a legal benchmark to see whether it could answer the question every team argues about in meetings: what RAG architecture should we use?*
...
That measurement problem is what [RAG Evaluator](https://github.com/fabrizioamort/RAG-evaluator) is for. This article is the platform's first full case study: three retrieval architectures, one legal benchmark with external gold labels, everything held fixed except the architecture. The case study produced three findings I did not fully expect — hybrid search *losing* to plain dense, a measurement bug that would have shipped a wrong conclusion, and an agentic retriever climbing past the paper's domain-tuned embedder without using embeddings at all. Each of those findings exists because of a specific platform capability, and that is the real point of this piece: the findings are the demo.
...
## The platform, in one section

I could have run this comparison as a benchmark script — a `for` loop over 100 questions, a FAISS index, a CSV at the end. That is how most of these comparisons get done, and it is exactly why most of them are not reproducible a week later: the embedding model lives in one variable, the chunk size in another, the judge prompt in a third, and when someone challenges your table you cannot say with confidence which knobs were set to what.

RAG Evaluator is a web platform (FastAPI + React, with a CLI sharing the same core) built around two ideas.
...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="../images/legal-rag-comparison-dark.png">
...
To test the platform I wanted a benchmark with external ground truth, a published baseline to calibrate against, and a domain hard enough to separate the architectures. In February, Isaacus published [Legal RAG Bench](https://isaacus.com/blog/legal-rag-bench): 4,876 passages from the Victorian Criminal Charge Book, 100 questions written by people who know the material, one gold supporting passage per question. The official harness indexes one passage as one document — no chunking games — and measures whether the gold passage id shows up in the top 5 retrieved. That is `hit@5`, the cleanest number in the whole exercise because no LLM judge touches it.
...
## What the platform caught first: my own number was wrong
...
Against dense's 53% that is +35 points. But the more interesting reference is the paper's own table: Isaacus needed Kanon 2, their domain-tuned legal embedder, to reach roughly 86% retrieval — the biggest lever in their study. The agent landed in the same territory with no embeddings at all. BM25, file listings, summaries, and the ability to keep reading.
...
- **Hybrid on legal text:** I would not. Sparse retrieval is a bet on lexical overlap, and this domain does not pay it out.
...
- **Gold access is not hit@5.** The agent's 88% counts a read of the gold passage at any point in its run; the vector systems face a rank-≤5 test. The comparison to Kanon 2's ~86% is directional — though the correctness column, which is metric-neutral, points the same way.
...
The case study wrote the roadmap. A fixed strong judge as a first-class evaluation option, not a prompt swap — the cleanest single upgrade to credibility. An open-book/closed-book toggle as a config knob, so generation policy becomes a measured variable instead of a buried prompt decision. A reranker stage on the dense path — the obvious lever the paper leaves on the table. And for the agent, the frontier is no longer accuracy but the bill: early-stop policies and history compaction to pull 15.9 million prompt tokens down, and a prep-time concept-alias index so vocabulary gaps get solved once at indexing time instead of per-question at 192 seconds.
...
*Fabrizio Amort is a GenAI architect. The numbers, config snapshots, and per-question traces in this piece were generated and exported by [RAG Evaluator](https://github.com/fabrizioamort/RAG-evaluator); every table is a direct export, manifest included. Code, docs, and the full comparison export live in the repo.*
