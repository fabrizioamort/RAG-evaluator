...
Short version: the two passive architectures landed where the paper says they should, their correctness moving with their retrieval. The third didn't climb over the ceiling so much as pick it up and carry it: 88% gold-passage access — above what the paper reaches with its own domain-tuned embedder — from an architecture that uses no embeddings at all. Then it sent me an itemized bill.
...
*All three runs completed generation and judging on all 100 questions. Retrieval is hit@5 on the gold passage id for the vector systems, and gold access — did the agent actually read the gold passage during its run — for the agent. Phase 1 numbers; caveats are at the bottom and they matter.*
...
I am telling this story because it is the entire argument for doing this on instrumented infrastructure. A script would have printed 38% and I would have written a worse, wronger article about how this stack underperforms the paper. The traces, the stored vectors, and the offline replay are what turned a wrong conclusion into a fixed bug. (One footnote for the careful reader: on the final rebuilt index used for the runs in this article, the same measurement reads 53% — a one-question wobble around the paper's 52%, which is the noise floor of a 100-question benchmark.)
...
- **Embeddings:** `text-embedding-3-large` at 3,072 dimensions for both vector systems. The agent uses none.
...
- **Agent budget:** the filesystem agent gets no top-k; it gets 40 reasoning iterations, 80 tool calls, and 40 file reads per question, and stops when it decides it has enough.
...
Two honest disclosures up front, because they change how you should read the table. First, the same model generated *and* judged. That is consistent across all three architectures — every run faces the identical judge — but a model grading its own homework is a known bias, so treat correctness and groundedness as directional, not as the paper's fixed GPT-5.2 verdict. Second, closed-book tethers correctness to what the retriever actually surfaced — if no retrieved passage holds the rule, the model must abstain. That is a feature for comparing retrievers fairly and a difference from the paper, which is effectively open-book.

## Result 1: hybrid lost to plain dense

This is the one I did not expect. Conventional wisdom says dense-plus-sparse hybrid with reciprocal rank fusion is a strict upgrade over dense alone — you get semantic matching *and* exact-term matching, fused. On this corpus it went the other way: hybrid retrieved the gold passage 41% of the time against dense's 53%. Adding SPLADE made retrieval worse.

I have a theory, and I am labelling it a theory because I did not chase it all the way down. Legal questions are vocabulary-poor relative to their answers — the named hypothetical does not lexically resemble the doctrinal passage that resolves it. A sparse retriever rewards shared terms, so it happily surfaces passages that share legal vocabulary ("evidence," "jury," "charge") without sharing the specific rule, and RRF then dilutes a strong dense signal with that noisier sparse one. On a corpus where lexical overlap tracked relevance, sparse would earn its keep. Here it pulled the wrong way.

The per-case signals add a second insult: hybrid didn't just find the gold passage less often, it did less with it when it did. Of hybrid's 41 gold hits, 26 became correct answers — a 63% conversion, against dense's 83% (44 of 53). RRF doesn't only cost you retrievals; the sparse side fills the rest of the top-5 with lexically-similar-but-wrong passages, so even a successful retrieval arrives wrapped in distractors. The lesson I am taking is narrower and more useful than "hybrid is bad": hybrid is a bet that lexical overlap signals relevance, and legal QA is close to the worst place to make that bet.

## Result 2: the passive retrievers are governed by their retrieval — with one honest wrinkle

Look at the two vector rows again. Hybrid: 41% retrieval, 39% correct — pinned, exactly the paper's thesis. Dense: 53% retrieval and 61% correct, eight points *above* its own hit rate, which a strict reading of the ceiling thesis says should not happen under a closed-book prompt.

The platform's per-case signals explain the overshoot without magic. Seventeen of dense's 61 correct answers came on questions where the gold passage was never retrieved at all, and in 21 cases the judge found the answer supported by a *different* retrieved passage than the official gold one. The Charge Book restates the same rule in more than one place — a charge document here, a commentary section there — so the single blessed gold id undercounts the evidence actually available in a top-5. The ceiling is real, but it is an *evidence* ceiling, not a gold-id ceiling, and `hit@5` against one gold passage is a lower bound on it.

The mechanism the paper describes still runs the show. A 12-point retrieval gap between dense and hybrid became a 22-point correctness gap, and abstention is almost the mirror image of retrieval quality — hybrid abstained 30 times, dense 18, the agent zero. When the top 5 holds no usable rule and the prompt is closed-book, an honest model says "I cannot answer this from the provided context" rather than confabulate. The chain is mechanical: bad retrieval → forced abstention → capped correctness. Swapping dense for hybrid did not change the model. It changed how often the model was handed the answer, and everything downstream followed. Both vector systems remain bounded by whatever landed in their five chunks — the question the next section answers is what happens when nothing is fixed at five.

## Result 3: the agent moved the ceiling

Now the filesystem RAG, and the reason this article has the title it does.

The agentic retriever does not get a top-5 budget. It treats the corpus as a filesystem — passages regrouped into documents, each with a generated summary and a BM25 index built at prep time — and it reads. It prefetches candidates, greps, pulls the most promising documents, reads their full text, and decides whether it has enough. Its retrieval metric isn't `hit@5` (there is no fixed-length ranked list to score), so I report `gold_accessed`: did it actually read the gold passage. That was **88%**.

Put that number next to the right comparison. Against dense's 53% it is +35 points. But the more interesting reference is the paper's own table: Isaacus needed Kanon 2, their domain-tuned legal embedder, to reach roughly 86% retrieval — the biggest lever in their study. The agent landed in the same territory with no embeddings at all. BM25, file listings, summaries, and the ability to keep reading.

That number did not come from the model, and it was not free. The first full agent run scored 59% gold access. What took it to 88% was reading traces and fixing what they showed, over two rounds: BM25 returning five overlapping windows of the *same* passage so the gold one sat crowded out at rank 7 (fix: dedupe by passage); the agent landing in the right section and satisficing on a sibling passage while the gold one sat unread in its own search results (fix: a mandated sibling sweep before finalizing); questions asking about "double jeopardy" when the corpus only ever says "punished more than once for the same act" (fix: a statutory-vocabulary reformulation rule); an answer emitted after a single file read (fix: a minimum-evidence guard). Same model, same corpus, same budget — 29 points of retrieval, all of it engineering. That is the precise sense in which the ceiling moved: for a passive index, retrieval quality is a property you *buy* when you pick an embedder; for an agent, it is a surface you can *work*.

Correctness followed: 82%, twenty-one points above dense. And the agent converts what it reads more reliably than either vector system — of the 88 questions where it read the gold passage, 79 ended correct, a 90% conversion against dense's 83% and hybrid's 63%. Full documents beat five chunks for the last mile too.

The abstention column deserves its own sentence: zero. Not rare — zero, in 100 questions, because an agent that can keep digging almost always finds something to stand on. It mostly converted that stubbornness into successes — 80 answers both correct *and* grounded, versus dense's 61 — but the same property is a liability worth naming. An agent that never says "I don't know" is one confident-wrong answer away from a problem, and its 13 ungrounded answers are the number I would watch in production, not its accuracy.

## The cost of thinking

Nothing is free, and this time the bill is itemized. The agent averaged 192 seconds per question against 6–7 for the vector systems — call it thirty times slower — and its worst single question ran almost 35 minutes. (All 100 completed; after the earlier runs the long tail is now a property you provision for, not a timeout you apologize for.) The money tells the same story at a different scale: the agent consumed 15.9 million prompt tokens across the run against roughly 180 thousand for each vector system, which works out to about 70× the cost per question — $0.015 against $0.0002.

Here is the thing, though: $0.015 is a cent and a half. The entire 100-question agent run cost $1.50 on a cheap model. The premium is enormous in relative terms and almost nothing in absolute ones, which is exactly what makes this an operational decision rather than a leaderboard:

- **High-volume, latency-sensitive, cost-sensitive** (a search box, an autocomplete, anything user-facing and synchronous): dense vector search. It is fast, it is cheap, and on this benchmark it tracked the paper.
- **High-value, low-volume, accuracy-critical** (a memo a lawyer will actually rely on, where three minutes and a cent and a half are nothing against being wrong): the agent earns its latency.
...
- **Closed-book policy.** It tethers correctness to retrieved evidence and makes the vector numbers strict. The paper is effectively open-book; it recovers retrieval misses from the model's parametric legal knowledge. That gap is policy, not pipeline.
...
- **Gold access is not hit@5.** The agent's 88% counts a read of the gold passage at any point in its run; the vector systems face a rank-≤5 test. The comparison to the paper's ~86% (Kanon 2) is directional, not apples-to-apples — though the correctness column, which is metric-neutral, points the same way.
- **Not a replication.** This is Legal RAG Bench used as a controlled harness to compare architectures on one stack, with the paper as a calibration reference. The one number I will put up against the paper is dense `hit@5` — 53.0% on the final index, 52.0% on the calibration run, against their 52%.

## What I'd change next

A fixed strong judge for every run, which is the cleanest single upgrade to credibility. A reranker on the dense path, which is the obvious lever the paper leaves on the table. An open-book toggle as a first-class config knob, so "answer from context only" versus "answer from context, fall back to your own legal knowledge" becomes a measured variable instead of a buried prompt decision. And for the agent, the frontier is no longer accuracy — it is the bill: early-stop policies and history compaction to pull those 15.9 million prompt tokens down, and a prep-time concept-alias index so vocabulary gaps get solved once at indexing time instead of per-question at 192 seconds.

But the finding I would lead with is the one in the title. The paper is right that retrieval is the ceiling — for retrievers that take what they're given. Hand the same corpus and the same models to a retriever that can read, reason, and read again, and the ceiling stops being a property of your embedding model and becomes a surface you can work: 59% to 88% gold access without touching the model, into territory the paper reaches only with a domain-tuned embedder. It is slower, it costs seventy times more per question (a cent and a half), and you have to watch it for overconfidence, because it never says "I don't know." It is also the only one of the three that read its way to the right passage nine times out of ten and answered four out of five legal questions correctly. On the kind of question where being right is the entire point, that trade looks a lot less like a luxury and a lot more like the job.
