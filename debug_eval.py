...
async def main():
    async with async_session_maker() as session:
        ...
        if eval_obj:
            ...
            # Check results
            query = select(EvaluationResult).where(EvaluationResult.evaluation_id == eval_id).limit(1)
            ...
            if res:
                ...
                if res.test_case_id:
                     ...
                     tc_res = await session.execute(select(TestCase).where(TestCase.id == res.test_case_id))
